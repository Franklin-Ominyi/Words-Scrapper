...
const scrambleWords = () => {
 ...
 stats[0].innerHTML = `<p>Number of words scanned: <span class="stat">${
  words.trim().split(" ").length
 }</span></p>`;
 stats[1].innerHTML = `<p>Number of characters scanned: <span class="stat">${
  words.trim().split("").length
 }</span></p>`;
...
// Script written by Franklin Ominyi (Altschooler)
